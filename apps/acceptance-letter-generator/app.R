...
# Optional navigation to the sibling ConferCraft app.
# Set CONFERCRAFT_CERTIFICATE_URL in the deployment environment to the
# deployed Certificate Generator URL.
CERTIFICATE_GENERATOR_URL <- Sys.getenv("CONFERCRAFT_CERTIFICATE_URL", unset = "")
...
certificate_link_class <- if (nzchar(CERTIFICATE_GENERATOR_URL)) {
  "tool-tab"
} else {
  ...
  tags$head(
    ...
    tags$style(
      HTML(
        ...
        html.confercraft-dark .tool-tab {
          color: #b9c6c2;
        }

        html.confercraft-dark .tool-tab:hover,
        html.confercraft-dark .tool-tab:focus {
          color: #73d3c3;
        }

        html.confercraft-dark .tool-tab-active {
          background: var(--teal);
          color: #ffffff !important;
        }
    ...
    div(
      ...
      div(
        ...
        div(
          ...
          tags$a(
            class = certificate_link_class,
            href = if (nzchar(CERTIFICATE_GENERATOR_URL)) CERTIFICATE_GENERATOR_URL else NULL,
            title = if (nzchar(CERTIFICATE_GENERATOR_URL)) {
              "Open Certificate Generator"
            } else {
              "Set CONFERCRAFT_CERTIFICATE_URL to enable this link"
            },
            `aria-disabled` = if (nzchar(CERTIFICATE_GENERATOR_URL)) "false" else "true",
            "Certificates"
          )
        ),
        div(
